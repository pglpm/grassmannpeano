...
%% Last-Updated: 2019-02-14T11:21:16+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
generally add up. Thus, if we add a flat with inner weight to itself, we
obtain a flat with twice the original weight; if we add a flat with outer
weight to itself, we obtain a flat with \emph{half} the original weight.

Note that the sum of the reciprocal of two weights -- in particular
parallel lengths, parallel areas, volumes -- is a well-defined geometric
construction even in the absence of a notion of distance, because it can be
computed in term of sums and ratios. For example, if $\yal$ and $\ybe$ are
parallel areas, their reciprocal sum is defined in terms of ratios as
\begin{equation}
  \label{eq:reciprocal_sum}
  \frac{1}{\frac{1}{\yal}+ \frac{1}{\ybe}} \defd
...
that has this same ratio with the area $\yal$.
\mynote{Picture}
...
segment with endpoints $C$, $A$ is $\abs{b/(a+b)}$ times the segment with
endpoints $A$, $B$, and the segment with endpoints $B$, $C$ is
$\abs{a/(a+b)}$ times the segment with endpoints $A$, $B$. A moment's
thought shows that these two requirements determine a unique location for
$C$. If the ratio $b/(a+b)$ is negative, then $C$ and $B$ lie on opposite
sides of $A$; analogously for the ratio $a/(a+b)$. \mynote{Pictures}
...
The sum of points with outer weight and inner sense
