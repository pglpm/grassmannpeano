...
%% Last-Updated: 2020-02-13T20:52:23+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\firstpublished}{18 April 2017}
\newcommand*{\propertitle}{The beauty of Gra{\ss}mann spaces [draft]}
...
%\\[2\jot] {\footnotesize\textnormal{\emph{line art will be by I. Bengtsson}}}%
}

%\date{Draft of 28 February 2019 (first drafted \firstdraft)}
...
\fi
\newcommand*{\dextro}{\mathord{\includegraphics[totalheight=\heightof{O}]{dextro.png}}}
\newcommand*{\laevo}{\mathord{\includegraphics[totalheight=\heightof{O}]{laevo.png}}}
...
attributed to \gm\ \citey{grassmann1844_r1878,grassmann1862}. Gra{\ss}mann's
...
Yet the basic ideas behind Gra{\ss}mann's work are quite intuitive and easy to
...
absence of a notion of distance (metric), the harmonic sum of parallel lengths,
...
{\centering
  \begin{tabular}{c|ccccccccc}
    ...
    $1$&&&$\zr\ywv$&$\zr0$&$1$ 
                                                &$\zb2$&$3$&$\zr\ywp$&$\zr\ywl$ \\
    $2$&&$\zr\ywv$&$\zr0$&$\zr1$&$2$ 
                                                &$3$&$\zr\ywp$&$\zr\ywl$&$\zr\ywa$ \\
    $3$&$\ywv$&$0$&$1$&$2$&$3$ 
    &$\ywp$&$\ywl$&$\ywa$&$\ywv$ \\
%    
    $\ywp$&$\zb0$&$\zb1$&$\zb2$&$3$&$\ywp$
    &$\zb\ywl$&$\zb\ywa$&$\zb\ywv$&\\
    $\ywl$&$\zb1$&$\zb2$&$3$&$\zr\ywp$&$\ywl$
    &$\zb\ywa$&$\zb\ywv$&&\\
    $\ywa$&$\zb2$&$3$&$\zr\ywp$&$\zr\ywl$&$\ywa$
    &$\zb\ywv$&&&\\
    $\ywv$&$3$&$\zr\ywp$ &$\zr\ywl$&$\zr\ywa$&$\ywv$
    &&&&
  \end{tabular}
...
\bigskip\mynote{The question whether we can replace the zero entries of the
  first two tables pivots around this problem:
  ...
  $(A_1 \ve\dotsb\ve A_4) \we A_5 \ne 0$. There is no way to make allowance
  for both these statements with one product only, even if
  non-associative.}

\clearpage

\mynote{flats with inner weight must \enquote{behave in a reciprocal way}:
  seen from limit forms of sum with themselves.}

\mynote{Contraction cannot be considered as a sort of division, because
  multiplication by scalar in numerator or denominator would lead to wrong
  multiple}
